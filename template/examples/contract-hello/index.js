const Web3 = require('web3');
const web3 = new Web3('http://localhost:9933');

const ABI = [{"inputs":[],"payable":false,"stateMutability":"nonpayable","type":"constructor"},{"anonymous":false,"inputs":[{"indexed":false,"internalType":"string","name":"message","type":"string"}],"name":"Said","type":"event"},{"constant":true,"inputs":[],"name":"owner","outputs":[{"internalType":"address","name":"","type":"address"}],"payable":false,"stateMutability":"view","type":"function"}];
const BYTECODE = "";

const main = async () => {
	// add account.
	const devPrivteKey = "0x60ed0dd24087f00faea4e2b556c74ebfa2f0e705f8169733b01530ce4c619883";
	const devAccount = web3.eth.accounts.privateKeyToAccount(devPrivteKey);

	const createTransaction = await web3.eth.accounts.signTransaction({
		from: devAccount.address,
		data: BYTECODE,
		value: "0x00",
		gasPrice: "0x10",
		gas: "0x100000",
	}, devAccount.privateKey);
	console.log(createTransaction);

	const createReceipt = await web3.eth.sendSignedTransaction(createTransaction.rawTransaction);
	console.log("Contract deployed at address", createReceipt.contractAddress);

	let contract = new web3.eth.Contract(ABI, createReceipt.contractAddress);
	let hello = await contract.methods.owner().call();
	console.log(hello)
};

main();
